...
function buildMonthSummary(config) {
  ...
  var sortedImages = s2.sort("system:time_start");
  ...
  var secondImageLabel = ee.String(ee.Algorithms.If(
    imageCount.gt(1),
    ee.Date(secondImage.get("system:time_start")).format("YYYY-MM-dd HH:mm"),
    ee.Algorithms.If(
      imageCount.gt(0),
      ee.String("fallback_first_image_").cat(ee.Date(secondImage.get("system:time_start")).format("YYYY-MM-dd HH:mm")),
      "no_valid_image"
    )
  ));
